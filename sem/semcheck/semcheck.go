...
func Check(file *ast.File) error {
	...
	f := func(node ast.Node) error {
		...
		case *ast.BasicLit:
			// if date constant is valid
			if node.Kind == token.DateLit {
				if err := checkDateTimeLit(node); err != nil {
					return err
				}
			}
		case *ast.ForStmt:
...
func checkInvalidExitStmt(exitStmt *ast.ExitStmt) error {
	...
	f = func(node ast.Node) error {
		...
		exitType := strings.ToLower(exitStmt.ExitType)
		switch stmt.(type) {
		case *ast.FuncDecl:
			if exitType == "function" {
				return nil
			}
		case *ast.SubDecl:
			if exitType == "sub" {
...
func checkReturnValueNotAssigned(call *ast.CallOrIndexExpr) error {
	...
	callType, err := call.Name.Decl.Type()
	if err != nil {
		return err
	}
	if callType == nil {
	...
	// check if function is a subroutine
	if _, ok := call.Name.Decl.(*ast.SubDecl); ok {
		return nil
	}
...
// checkDateTimeLit reports an error if the given date constant is invalid.
func checkDateTimeLit(lit *ast.BasicLit) error {
	// ensure date constant is valid
	value := fmt.Sprint(lit.Value)
	lit.Value = strings.Trim(value, "#")
	...
	if DatePart != "" {
		...
		if err != nil {
			_, err := time.Parse("2006/01/02", DatePart)
			if err != nil {
				return errors.Newf(lit.Token().Position, "invalid date constant %q", lit.Value)
			}
		}
	}
	// check time part
	if TimePart != "" {
		...
		if err != nil {
			return errors.Newf(lit.Token().Position, "invalid time constant %q", lit.Value)
		}
	}
