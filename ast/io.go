...
// ReadCompact reads a string and returns a ElseIfStmt
func (n *CaseExpr) ReadCompact(s *CharStack, d *IdList) error {
	var err error
	var node Node
	var ok bool
	// read token
...
func (n *CaseExpr) WriteCompact(d *Dict) string {
	buf := strings.Builder{}
...
func (n *ExitStmt) ReadCompact(s *CharStack, d *IdList) error {
	...
	var nExitType int
	fmt.Sscanf(s.String(), "%X", &nExitType)
	n.ExitType = d.get(nExitType)
	// remove string from stack
	if !s.PopVerifyString(fmt.Sprintf("%X", nExitType)) {
		return fmt.Errorf("error reading ExitStmt")
	}
...
func (n *ExitStmt) WriteCompact(d *Dict) string {
	v := d.get(n.ExitType)
	return fmt.Sprintf("[%X:%s,%X]", ExitStmtValue, WriteCompactToken(n.ExitKw, d), v)
}
...
func (n *ForStmt) WriteCompact(d *Dict) string {
	...
	buf.WriteString(n.ForExpr.WriteCompact(d))
	buf.WriteString("(")
	for _, stmt := range n.Body {
		buf.WriteString(stmt.WriteCompact(d))
	}
...
func (n *EnumDecl) WriteCompact(d *Dict) string {
	...
	buf.WriteString(n.EnumName.WriteCompact(d))
	buf.WriteString("(")
...
func (n *CallSubStmt) ReadCompact(s *CharStack, d *IdList) error {
	...
	n.Name, ok = node.(*Identifier)
	if !ok {
		return fmt.Errorf("error reading CallSubStmt.Name")
	}
	// skip two parenthesis
	if !s.PopVerifyString("(") {
...
func (n *CallSubStmt) WriteCompact(d *Dict) string {
	...
	buf.WriteString(n.Name.WriteCompact(d))
	buf.WriteString("(")
	for _, arg := range n.Args {
		buf.WriteString(arg.WriteCompact(d))
	}
...
func (n *ForNextExpr) WriteCompact(d *Dict) string {
	...
	buf.WriteString(n.Var.WriteCompact(d))
...
func (n *UserDefinedType) WriteCompact(d *Dict) string {
	return fmt.Sprintf("[%X:%v]", UserDefinedTypeValue, n.Name.WriteCompact(d))
}
