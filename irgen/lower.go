...
const ArrayVarSizeName = ".%s_%d" // array name, dimension
...
// forStmt lowers the given for statement to LLVM IR, emitting code to f.
func (m *Module) forStmt(f *Function, stmt *ast.ForStmt) {
	bodyBranch := f.NewBlock("")
	condBranch := f.NewBlock("")
	incrBranch := f.NewBlock("")
	endBranch := f.NewBlock("")

	// Initialize the loop
	var cond value.Value
	var iterator value.Value
	switch s := stmt.ForExpression.(type) {
	case *ast.ForNextExpr:
		iterator = m.initForNext(f, s)
	case *ast.ForEachExpr:
		iterator = m.initForEach(f, s)
	default:
		panic(fmt.Sprintf("support for %T not yet implemented", stmt.ForExpression))
	}
	termBr := ir.NewBr(condBranch.Block)
	f.currentBlock.SetTerm(termBr)
	...
	switch s := stmt.ForExpression.(type) {
	case *ast.ForNextExpr:
		cond = m.conditionForNext(f, s, &iterator)
	case *ast.ForEachExpr:
		cond = m.conditionForEach(f, s, &iterator)
	default:
		panic(fmt.Sprintf("support for %T not yet implemented", stmt.ForExpression))
	}
	termCondBr := ir.NewCondBr(cond, bodyBranch.Block, endBranch.Block)
	...
	// basic block is none nil).
	if f.currentBlock != nil {
		termBr := ir.NewBr(incrBranch.Block)
		f.currentBlock.SetTerm(termBr)
	}

	// Increment:
	f.currentBlock = incrBranch
	switch s := stmt.ForExpression.(type) {
	case *ast.ForNextExpr:
		m.incrementForNext(f, s, &iterator)
	case *ast.ForEachExpr:
		m.incrementForEach(f, s, &iterator)
	default:
		panic(fmt.Sprintf("support for %T not yet implemented", stmt.ForExpression))
	}
	termBr = ir.NewBr(condBranch.Block)
...
// initForNext lowers the given for-next initalization condition to LLVM IR, emitting code to f.
func (m *Module) initForNext(f *Function, expr *ast.ForNextExpr) value.Value {
	// Input:
	//    for i = 0 to 10
	// Output:
	//    %i = alloca i32
	...
	// Initialize iterator.
	iterator := m.ident(f, expr.Variable)
	init := m.expr(f, expr.From)
	init = m.pointerToValue(f, init)
	init = m.convert(f, init, iterator.Type())
	f.currentBlock.NewStore(init, iterator)
	return iterator
}

// initForEach lowers the given for-each initalization condition to LLVM IR, emitting code to f.
func (m *Module) initForEach(f *Function, expr *ast.ForEachExpr) value.Value {
	// Input:
	//    for each i in arr
	// Output:
	//    %i = alloca i32
	...
	//    br i1 %cond, label %body, label %end
	// Create iterator.
	iterator := f.currentBlock.NewAlloca(irtypes.I32)
	// Initialize iterator.
	f.currentBlock.NewStore(constZero(irtypes.I32), iterator)
	// Load array value in variable
	variable := m.ident(f, expr.Variable)
	array := m.expr(f, expr.Collection)
	gep := f.currentBlock.NewGetElementPtr(array.Type(), array, iterator)
	value := f.currentBlock.NewLoad(variable.Type(), gep)
	f.currentBlock.NewStore(value, variable)
	return iterator
}

// conditionForNext lowers the given for-next condition to LLVM IR, emitting code to f.
func (m *Module) conditionForNext(f *Function, expr *ast.ForNextExpr, iterator *value.Value) value.Value {
	// Input:
	//    for i = 0 to 10
	// Output:
	//    %i = load i32, i32* %i
	...
	//    br i1 %cond, label %body, label %end
	// Load iterator.
	iteratorVal := f.currentBlock.NewLoad(irtypes.I32, *iterator)
	// get step value
	stepValue := 1.0
	if expr.Step != nil {
		env := object.NewEnvironment()
		step := eval.Eval(nil, expr.Step, env)
		var err error
		stepValue, err = strconv.ParseFloat(step.String(), 64)
		if err != nil {
			panic(fmt.Sprintf("unable to parse step value; %v", err))
		}
	}

	// get end value
	to := m.expr(f, expr.To)
	to = m.pointerToValue(f, to)
	to = m.convert(f, to, iteratorVal.Type())

	// validate condition
	var cond value.Value
	if stepValue > 0 {
		cond = f.currentBlock.NewICmp(enum.IPredSLT, iteratorVal, to)
	} else {
		cond = f.currentBlock.NewICmp(enum.IPredSGT, iteratorVal, to)
	}
	return cond
}

// conditionForEach lowers the given for-each condition to LLVM IR, emitting code to f.
func (m *Module) conditionForEach(f *Function, expr *ast.ForEachExpr, iterator *value.Value) value.Value {
	// Input:
	//    for each i in arr
	// Output:
	//    %i = load i32, i32* %i
	...
	// collection, ok := expr.Collection.(*ast.CallOrIndexExpr)
	// if !ok {
	// 	panic("expected identifier")
	// }
	// scope := m.getDeclarationScope(collection.Identifier)

	// Create condition.
	// arrayLengthName := fmt.Sprintf(ArrayVarSizeName, collection.Identifier.Name, 0)
	// var arrayLength value.Value
	// if scope == "" {
	// 	arrayLength = m.LookupGlobal(arrayLengthName)
	// } else {
	// 	arrayLength = f.currentBlock.emitLocal()
	// }

	// length := f.currentBlock.NewLoad(irtypes.I32, arrayLength)
	// cond := f.currentBlock.NewICmp(enum.IPredSLT, iteratorVal, length)
	// return cond
	return nil
}

// incrementForNext lowers the given for-next increment to LLVM IR, emitting code to f.
func (m *Module) incrementForNext(f *Function, expr *ast.ForNextExpr, iterator *value.Value) {
	// Input:
	//    for i = 0 to 10
	// Output:
	//    %i = load i32, i32* %i
	...
	iType, ok := iteratorType.(*irtypes.PointerType)
	if ok {
		iteratorType = iType.ElemType
	}
	...
	var stepVar value.Value
	if iteratorType.Equal(irtypes.I32) || iteratorType.Equal(irtypes.I64) {
		stepValue = math.Round(stepValue)
		stepVar = constant.NewInt(iteratorType.(*irtypes.IntType), int64(stepValue))
	} else {
		stepVar = constant.NewFloat(iteratorType.(*irtypes.FloatType), stepValue)
	}

	// Load iterator.
	var iteratorVal value.Value
	iteratorVal = f.currentBlock.NewLoad(iteratorType, *iterator)
	// Increment iterator.
	step := m.convert(f, stepVar, iteratorType)
	if iteratorType.Equal(irtypes.I32) || iteratorType.Equal(irtypes.I64) {
		iteratorVal = f.currentBlock.NewAdd(iteratorVal, step)
	} else {
		iteratorVal = f.currentBlock.NewFAdd(iteratorVal, step)
	}
	f.currentBlock.NewStore(iteratorVal, *iterator)
}

// incrementForEach lowers the given for-each increment to LLVM IR, emitting code to f.
func (m *Module) incrementForEach(f *Function, expr *ast.ForEachExpr, iterator *value.Value) {
	// Input:
	//    for each i in arr
	// Output:
	//    %i = load i32, i32* %i
	...
	iteratorVal = f.currentBlock.NewLoad(irtypes.I32, *iterator)
	// Increment iterator.
	iteratorVal = f.currentBlock.NewAdd(iteratorVal, constant.NewInt(irtypes.I32, 1))
	f.currentBlock.NewStore(iteratorVal, *iterator)
