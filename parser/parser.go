...
func (p *Parser) ParseParamItem() *ast.ParamItem {
	...
	if p.curTokenIs(token.Assign) {
		p.nextToken()
		param.VarValue = p.ParseExpression()
...
func (p *Parser) ParseExitStatement(inSub bool, inFunction bool) *ast.ExitStmt {
	...
	switch p.currentToken().Kind {
	case token.KwDo:
		stmt.ExitType = p.currentToken().Literal
	case token.KwFor:
		stmt.ExitType = p.currentToken().Literal
	case token.KwFunction:
		if inFunction {
...
func (p *Parser) ParseForStatement(inSub bool, inFunction bool) *ast.ForStmt {
	...
	if p.curTokenIs(token.Identifier) && p.peekTokenIs(1, token.Assign) {
		stmt.ForExpr = p.ParseForVariableStatement(inSub, inFunction)
		if stmt.ForExpr == nil {
			// p.AddError(p.currentToken(), "expected for variable iteration")
			return nil
		}
	} else if p.curTokenIs(token.KwEach) {
		stmt.ForExpr = p.ParseForEachStatement(inSub, inFunction)
		if stmt.ForExpr == nil {
...
//		Parent Node
//	}
func (p *Parser) ParseSelectBody(inSub bool, inFunction bool) []ast.CaseExpr {
	block := make([]ast.CaseExpr, 0)
	gotElse := false
	for {
		stmt := &ast.CaseExpr{CaseKw: p.currentToken()}
		// skip case keyword
		if !p.curTokenIs(token.KwCase) {
			p.AddError(p.currentToken(), "expected 'case'")
			return nil
