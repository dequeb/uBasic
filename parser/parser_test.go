...
func testForStatement(t *testing.T, s ast.Node, expression string, statements []string) bool {
	...
	if Stmt.ForExpr.String() != expression {
		t.Errorf("For Stmt.Condition.String()   not '%s'. got=%s", expression, Stmt.ForExpr.String())
		return false
	}
...
func testSubCallStatement(t *testing.T, s ast.Node, subName string, params []string) bool {
	...
	if Stmt.Name.Name != subName {
		t.Errorf("SubCall.Name   not '%s'. got=%s", subName, Stmt.Name.Name)
		return false
	}

	if len(Stmt.Args) != len(params) {
		t.Errorf("SubCall.Args length  not '%d'. got=%d", len(params), len(Stmt.Args))
		return false
	}
